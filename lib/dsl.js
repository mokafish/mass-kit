...
export class Evaluator {
     ...
     */
    register(node) {
        this.calculator.set(node, this.processor[node.opcode](...node.data));
    }
    ...
    evaluate(node, envs, currentEnv) {
        ...
        const calc = this.calculator.get(node);
        if (!calc) {
            throw new Error(`No registered for node: ${node}`);
        }

        // Evaluate the process unit
        const result = calc.tick();
        
        // If the result is an overflow, we can handle it here
        if (calc.overflow) {
            // Handle overflow logic if needed
            console.warn(`Overflow occurred for node: ${node.opcode}`);
        }

        // Store the result in the runtime environment
        // envs[currentEnv].set(node, result);
        return result;
    }
}
...
export class Interpreter {
    constructor(
        ...
        this.evaluator = new Evaluator(processor);
        this.environment = {}
    }
    ...
    interpret() {
        for (const scope in this.environment) {
            const runtime = this.environment[scope];
            for (const node of runtime.symbolTable.values()) {
                // Evaluate each node in the current scope
                const value = this.evaluator.evaluate(node, this.environment, scope);
                // Store the result in the runtime environment
