...
export class Runtime {
    ...
    evaluate(id) {
        ...
        if (item.pow) {
            ...
            console.log(`Evaluating item ${id} with pow ${item.pow}`);
            console.log(`  Pow item value: ${powItem.value}`);
            console.log(`  Pow item overflow: ${powItem.calculator.overflow}`);
            if (powItem.calculator.overflow) {
                item.value = item.calculator.tick(this);
            }
        } else {
            item.value = item.calculator.tick(this);
        }
...
export class Interpreter {
    ...
    load(input, scope = 'main') {
        if (this.parser === undefined) {
            throw new Interpreter.InterpreterError('Interpreter is readied. Stop load inputs.');
        }
    ...
    ready() {
        try {
            ...
            if (e.code === 'CYCLE_DETECTED') {
                throw new Interpreter.InterpreterError('Circular references exist between tags.');
            }
            throw e
        }
...
export const builtinProcessor = {
    ...
    ref: (target) => {
        return {
            overflow: true,
            tick: (runtime) => {
                return runtime.heapGet(target).value
            }
        }
    },
    echo: (value) => {
