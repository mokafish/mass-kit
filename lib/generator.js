...
export function power(iterable, startExponent, endExponent, separator) {
    ...
    let currentTicker = makeProductTicker(currentExponent, iterableArray, separator);
    ...
    return function ticker() {
        ...
        if (result.overflow) {
            ...
            if (currentExponent > endExponent) {
                ...
                retOverflow = true; // This call returns overflow=true due to reset
            }
            // Create new product ticker for the new exponent
            currentTicker = makeProductTicker(currentExponent, iterableArray, separator);
        ...
        return {
            value: result.value,
            overflow: retOverflow
        };
    };
}

/**
 * Helper function to create a product ticker for a given exponent.
 * @param {number} exponent - The exponent (number of times to repeat the iterable).
 * @param {Array} iterable - The input iterable as an array.
 * @param {string|null} separator - Separator for joining combinations.
 * @returns {Ticker} A product ticker function.
 */
function makeProductTicker(exponent, iterable, separator) {
    // Create an array of `exponent` copies of the iterable
    const iterables = Array(exponent).fill(iterable);
    return product(iterables, separator);
}